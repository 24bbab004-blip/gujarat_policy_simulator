...
html,.stApp,section.main{color-scheme:light!important}.stApp{background:radial-gradient(circle at 92% 7%,rgba(33,161,137,.14),transparent 25%),radial-gradient(circle at 4% 56%,rgba(58,117,196,.11),transparent 23%),linear-gradient(140deg,#f4f9ff 0%,#f8fbf8 54%,#f4f3ff 100%);background-attachment:fixed;font-family:'DM Sans',sans-serif;color:#172b4d}
...
.brand{display:flex;align-items:center;gap:.7rem;margin:.3rem 0 1.2rem}.brand-mark{width:42px;height:42px;border-radius:13px;background:linear-gradient(145deg,#f7c55f,#f28d4b);position:relative;box-shadow:0 7px 17px rgba(0,0,0,.2)}.brand-mark:before{content:'';position:absolute;left:10px;top:9px;width:19px;height:19px;border:3px solid #083b56;border-radius:6px;transform:rotate(45deg)}.brand-mark:after{content:'';position:absolute;left:17px;top:16px;width:7px;height:7px;background:#083b56;border-radius:50%}.brand-title{font-weight:800;font-size:1.05rem;line-height:1.05}.brand-sub{font-size:.66rem!important;letter-spacing:.12em;font-weight:700;opacity:.75;margin-top:.16rem}
...
/* Number fields must stay dark on the light Streamlit theme. */
[data-testid="stNumberInput"] input,[data-testid="stNumberInput"] input[type="number"]{color:#103f5a!important;-webkit-text-fill-color:#103f5a!important;opacity:1!important;visibility:visible!important;font-weight:600!important}
